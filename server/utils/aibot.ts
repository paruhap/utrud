...
async function fetchAiResponse(prompt: string): Promise<string> {
  try {
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        "model": "deepseek/deepseek-chat-v3-0324:free", // Меняем модель на более быструю mistralai/mixtral-8x7b-instruct
        "messages": [
          {
            "role": "user",
            "content": prompt
    ...
    return `${aiResponse}\n\n---\nВнимание! Я бот на основе ИИ. Для получения юридической консультации рекомендую обратиться к специалисту по трудовому праву: @${process.env.STAFF_USERNAME}`;
